...
local countdown_display_maiden = require("graphics.countdown_display_maiden")
...
function maidenmain.update(menu, current_position, ChestsInteractor, Movement, explorer_circle_radius)
    ...
    if not maidenmain.menu_elements.main_helltide_maiden_auto_plugin_enabled:get() then
        console.print("Plugin Maidenmain está desativado")
        countdown_display_maiden.stop_countdown()
        return "disabled"
    end

    local game_state = GameStateChecker.check_game_state()
    if game_state ~= "helltide" then
        console.print("Não está em Helltide. Desativando plugin Maidenmain.")
    ...
    if helltide_start_time == 0 or not helltide_origin_city then
        if maidenmain.determine_helltide_origin_city() then
            ...
            countdown_display_maiden.start_countdown()
    ...
    local remaining_time = maidenmain.get_remaining_helltide_duration()
    if remaining_time <= 0 then
        if not maidenmain.switching_to_chest_farming then
            console.print("Duração da Maiden expirou. Mudando para farming de baús.")
    ...
    if circular_movement.is_near_maiden(player_position, maidenmain.helltide_final_maidenpos, maidenmain.explorer_circle_radius) then
        ...
        --console.print("Muito longe da Maiden. Pulando movimento circular.")
    end
    ...
    if helltide_start_time == 0 or not helltide_origin_city then
        if maidenmain.determine_helltide_origin_city() then
            ...
            if waypoints then
                Movement.set_waypoints(waypoints)
                console.print("Waypoints carregados para a cidade de origem da Helltide: " .. helltide_origin_city)
                local remaining_duration = maidenmain.get_remaining_helltide_duration()
                console.print("Tempo restante da Helltide: " .. math.floor(remaining_duration / 60) .. " minutos e " .. math.floor(remaining_duration % 60) .. " segundos")
            else
                console.print("Falha ao carregar waypoints para a cidade de origem da Helltide.")
                return "error"
            end
...
function maidenmain.render()
    ...
    if maidenmain.menu_elements.main_helltide_maiden_auto_plugin_show_task:get() then
        -- Implementar lógica de exibição de tarefa aqui
        -- Por exemplo:
        -- local task = "Tarefa atual: Explorando"
        -- graphics.draw_text(vec2:new(10, 10), task, color.new(255, 255, 255, 255))
    end

    if countdown_display_maiden.is_active() then
        local remaining_time = maidenmain.get_remaining_helltide_duration()
        countdown_display_maiden.update_and_draw(remaining_time)
    end
end

function maidenmain.get_remaining_helltide_duration()
    local current_time = get_time_since_inject()
    local total_duration = maidenmain.menu_elements.main_helltide_maiden_duration:get() * 60 -- Converte minutos para segundos
    local elapsed_time = current_time - helltide_start_time
    local remaining_time = total_duration - elapsed_time
    
    return math.max(0, remaining_time) -- Garante que o tempo restante não seja negativo
end
